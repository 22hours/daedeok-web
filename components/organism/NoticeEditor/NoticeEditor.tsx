...
import style from "./NoticeEditor.module.scss";
...
const NoticeEditorContorller = (props: ControllerProps) => {
    ...
    const handleSubmit = async () => {
        const res = await clientSideApi("POST", "MAIN", "TOTAL_NOTICE_NEW", undefined, {
            title: title.value,
            content: editorController.getMarkdownContent(),
        });
        if (res.result === "SUCCESS") {
            router.push(`/acinfo/notice/detail/${res.data}`);
        } else {
            alert(res.msg);
        }
    };
...
const NoticeEditor = (props: Props) => {
    return (
        <WysiwygEditorProvider>
            <NoticeEditorContorller {...props} />
        </WysiwygEditorProvider>
    );
};
